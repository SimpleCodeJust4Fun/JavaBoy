package cn.jianjie.javaboy.gpu;

import cn.jianjie.javaboy.gpu.phase.*;
import cn.jianjie.javaboy.memory.Dma;
import cn.jianjie.javaboy.memory.Ram;
import cn.jianjie.javaboy.AddressSpace;
import cn.jianjie.javaboy.cpu.InterruptManager;
import cn.jianjie.javaboy.cpu.InterruptManager.InterruptType;
import cn.jianjie.javaboy.gpu.phase.*;
...
public class Gpu implements AddressSpace {
    ...
    public Mode tick() {
        ...
        if (phase.tick()) {
            ...
            switch (oldMode) {
                ...
                case HBlank:
                    ticksInLine = 0;
                    if(r.get(GpuRegister.WX) < 166 && r.get(GpuRegister.WY) < 143 && r.get(GpuRegister.LY) >= r.get(GpuRegister.WY)
    ...
    private int getStat() {
        return r.get(GpuRegister.STAT) | mode.ordinal() | (r.get(GpuRegister.LYC) == r.get(GpuRegister.LY) ? (1 << 2) : 0) | 0x80;
    }
